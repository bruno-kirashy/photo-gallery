    ...
    return(
        <ul className="flex items-center justify-center flex-wrap ">
            ...
            <li className="flex items-center justify-center flex-col cursor-pointer mx-2" key={index}>
                <div className="w-full h-full aspect-video mx-2 md:mx-0 mb-2">
                    <img 
                    onClick={() => handleClickPhoto(item.img)}
                    src={`/Images/${item.img}.jpg`} 
                    alt={item.description} 
                    className={`w-full h-full object-cover md:w-90 md:h-50 border-1 transition-all duration-500 rounded-md hover:opacity-100 hover:shadow-2xl shadow-blue-500/10 hover:scale-103 ${darkMode ? 'border-gray-600/20 opacity-90' : 'border-gray-900/50 shadow-md shadow-black/20'}`}
                    />
                </div>
